...
import { setIsPakageCreated } from '../redux/slices/pakageSlice';
import { createPakage } from '../redux/actions/pakageAction';
...
const CreatePakage = () => {
  ...
  const { isPakageCreated, isLoading } = useSelector((state) => state.pakageState);
  ...
  useEffect(() => {
    if (isPakageCreated) {
      ...
      dispatch(setIsPakageCreated(false));
    }
  }, [isPakageCreated, dispatch, navigate]);
  ...
  const handleSubmit = (event) => {
    ...
    if (!name || !location || !distance || !description || specification.length === 0) {
    ...
    dispatch(createPakage(formData));
  }

  return (
    <Fragment>
      <Meta title="Create Package_Serandib Travel" />
      <div className="flex">
        <SideBar />
        {isLoading ? <Loader /> : (
          <div className="px-4 md:px-10 lg:px-20 xl:px-48 mx-auto">
            <h2 className="text-2xl font-medium text-center my-8">Create Package</h2>
