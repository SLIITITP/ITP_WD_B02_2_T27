...
import { deleteHotel, getPakageAction, uploadHotelPicture } from '../redux/actions/pakageAction';
...
const PakageHotels = () => {
    ...
    const { id } = useParams();
    const { isLoading, pakage } = useSelector((state) => state.pakageState);
    ...
    const rowsPerPage = 5;
    const emptyRows = Math.max(0, (1 + page) * rowsPerPage - pakage?.hotels.length);
    ...
    return (
        <Fragment>
            <Meta title={`${pakage?.name}'s Hotels`} />
            <div className="flex">
                <SideBar />
                <Fragment>
                    {isLoading ? <Loader /> : (
                                ...
                                <div className="w-[80%] sm:w-[60%] md:w-[70%] mx-auto mt-3">
                                    <div className="flex flex-col md:flex-row gap-6 md:gap-4 justify-between">
                                        <div className="flex gap-4">
                                            <Button onClick={() => navigate('/admin/pakages')} variant="contained" className="!text-gray-100 !bg-blue-400"><ArrowBackIosNewIcon fontSize="small" className="mr-2" />Back</Button>
                                            <Button onClick={() => navigate(`/admin/pakage/${id}/hotel/new`)} variant="outlined" className="!border-blue-400 !text-blue-400"><WeekendIcon fontSize="small" className="mr-2" />Add Hotel</Button>
                                        </div>
                                        <div>
                                            <div className="flex gap-4">
                                        ...
                                        <Table className="min-w-[700px]">
                                            <TableHead >
                                                <TableRow className="bg-blue-300">
                                                    ...
                                                    <TableCell align="center">Pakage No</TableCell>
                                            ...
                                            <TableBody>
                                                {(rowsPerPage > 2 ? pakage?.hotels.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) : pakage.hotels)?.map((hotel) => (
                                                    <TableRow key={hotel._id} style={{ height: 72.8 }}>
                                                        ...
                                                        <TableCell align="center">{hotel.number}</TableCell>
                                    ...
                                    <Dialog open={isDeleteOpen}>
                                        <div className="p-4" >
                                            ...
                                            <DialogContent className="m-8">
                                                <DialogContentText className="text-gray-900">This will delete the Package's booking details also.</DialogContentText>
                                            </DialogContent>
